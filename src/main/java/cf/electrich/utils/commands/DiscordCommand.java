...
public class DiscordCommand implements CommandExecutor {
    ...
    public DiscordCommand(ElectricUtils plugin){
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String label, String[] args) {
        if(!(commandSender instanceof Player)) {
            ...
            String path = "DISCORD-COMMAND-ENABLED";

            if (language.contains("SOCIALS.DISCORD-MESSAGE")) {
                if (settings.getString(path).equals("true")) {
                    List<String> message = language.getStringList("SOCIALS.DISCORD-MESSAGE");
